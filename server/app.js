...
// Blocks until each request finishes
app.get('/sync-write', (req, res) => {
  const stopPerf = startPerf(req.url);
  writeFileSync(`/tmp/${Date.now()}.json`, 'hello'.repeat(50_000_000));
  stopPerf();
...
app.get('/async-write', async (req, res) => {
  const stopPerf = startPerf(req.url);
  await writeFile(`/tmp/${Date.now()}.json`, 'hello'.repeat(50_000_000));
  stopPerf();
  res.json({ message: 'written' });
});
...
app.get('/async-fork', (req, res) => {
  // Creates a new process
  const stopPerf = startPerf(req.url);
  const child = fork('./server/fork.js');
...
// than a different process. This uses less memory than forking a process, but
// would want to use a worker pool in a real app since too many workers can
// also use too many resources.
app.get('/async-worker', (req, res) => {
